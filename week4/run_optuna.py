...
from datasets.datasets_IKER import TripletCOCO_B, TripletCOCO_A
...
def objective_model_cv(trial):
    params = {
        ...
        'lr': trial.suggest_categorical('lr', [0.001]),  # 0.0001, 0.001, 0.01, 0.1, 0.2, 0.3
        ...
        'text_encoder': trial.suggest_categorical('text_encoder', ['FASTTEXT']), # FASTTEXT / BERT
    }

    config = dict(trial.params)
    config['trial.number'] = trial.number

    execution_name = f'{str(id)}_TASK_{str(params["task"])}'
    wandb.init(
        project='C5_w4_BERT',
        entity='c3_mcv',
        name=execution_name,
    ...
    if params['task'] == 'B':
        ...
        if params['text_encoder'] == 'FASTTEXT':
            ...
            model = TextImageNet(ImageNet(embedding_dim=params['output']), TorchTextNetBert(embedding_dim=params['output'], device=device), sm.euclidean)
